...
export default function ProfileContent({ user }: ProfileContentProps) {
  ...
  const tabTitle = {
    account: { label: t('tabs.account'), icon: <UserCircle2 className="h-5 w-5 text-maroon-600 dark:text-softpink-400" /> },
    password: { label: t('tabs.password'), icon: <KeyRound className="h-5 w-5 text-maroon-600 dark:text-softpink-400" /> },
  };
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-700 dark:bg-zinc-950">
      ...
      <div className="-mt-16 flex flex-col items-center px-6 pb-2">
        <ProfileAvatar photo={user.photo} name={`${user.firstName} ${user.lastName}`} />
        <h2 className="mt-3 text-xl font-bold text-zinc-800 dark:text-zinc-50">
          {user.firstName} {user.lastName}
        </h2>
        <p className="text-sm text-zinc-400 dark:text-zinc-500">@{user.username}</p>
      ...
      <div className="flex flex-col gap-6 px-6 pb-8 sm:flex-row">
        ...
        <div className="flex-1">
          ...
          {activeTab === 'password' && <ChangePasswordForm email={user.email} />}
        </div>
      </div>
    </div>
  );
